...
from math import sqrt,acosh,asinh,radians
...
for k in range(len(resolution_list)):
    ...
    for i in range(len(e_list)):
        ...
        for j in range(len(theta_list)):
            ...
            rot_mat = rotation_matrix_by_raxis_angle([0, 0, 1], radians(theta))
            grid.crop_rotated_ellipse(size / 2, [R1, R2, R3], rot_mat, 2)
            # grid.crop_cylinder_outside_frictive(size/2, [L1,L2], 3)
            os.mkdir(f'./{geom}')
            grid.output(f'./{geom}/model.vtk', 'amitex_fftp')
            shutil.copy('../../xml_files/material_coalescence_m_0.1.xml', f'./{geom}/material.xml')
            shutil.copy('../../xml_files/algorithm_coalescence.xml', f'./{geom}/algorithm.xml')
            shutil.copy('../../xml_files/load_coalescence_tension.xml', f'./{geom}/load.xml')
            print(f'finish {geom}\n')
            ...
            # os.mkdir(f'./{geom}')
            # grid.output(f'./{geom}/model.vtk', 'amitex_fftp')
            # shutil.copy('../../xml_files/mises.xml', f'./{geom}/material.xml')
            # shutil.copy('../../xml_files/onestep.xml', f'./{geom}/algorithm.xml')
            # shutil.copy('../../xml_files/tension.xml', f'./{geom}/load.xml')
            # print(f'finish {geom}\n')
